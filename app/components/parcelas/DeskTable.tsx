...
export default function DeskTable({
  ...
  return (
    ...
    <div className="max-w-6xl mx-auto p-6">
      <TopBarDesk query={query} setQuery={setQuery} tipoBarra='parcela' />
          ...
          <div className="overflow-x-auto flex-grow">
            <table className="min-w-full bg-white text-sm">
              <thead className="bg-slate-700">
                <tr>
                  <th className="px-6 py-3 text-center font-semibold text-white">Numero de sectores</th>
                  <th className="px-6 py-3 text-center font-semibold text-white">Nombre</th>
                  <th className="px-6 py-3 text-center font-semibold text-white">Numero de parcela</th>
              ...
              <tbody>
                {data.length > 0 ? (
                  data.map((item) => (
                    ...
                    >
                      <td className="px-6 py-3 text-center text-gray-800">{item.n_sectores}</td>
                      <td className="px-6 py-3 text-center text-gray-800">{item.nombre}</td>
                      <td className="px-6 py-3 text-center text-gray-600">{item.numero_parcela}</td>
                      <td className="px-6 py-3 text-center">
                        <div className="flex justify-center gap-2">
                          <Link
                            href={`/dashboard/parcelas/${item.id}/mostrar`}
